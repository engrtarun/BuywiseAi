...
function ChatHeader({ isSidebarOpen, onMenuToggle, isGuest, isTemporaryChat, onNewTemporaryChat }: ChatHeaderProps) {
  return (
    <header className="shrink-0 z-[60] relative flex items-center bg-ink-deeper px-4 py-3 border-b border-line-ondark h-14">
      <div className="w-full flex items-center justify-between">
        <div className="flex items-center gap-3">
          <HamburgerButton isOpen={isSidebarOpen} onClick={onMenuToggle} />
          {/* Subtle Chat Indicator Area */}
          {isTemporaryChat && (
            <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/[0.04] border border-white/10 text-xs font-medium text-text-ondark select-none">
